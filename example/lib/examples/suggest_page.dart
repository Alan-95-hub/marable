...
import 'package:yandex_mapkit_example/examples/widgets/suggestions_table.dart';
...
class _SuggestionsExampleState extends State<_SuggestionsExample> {
              ...
              Expanded(
                ...
                if (results != null)
                  SearchSuggestionsTable(
                    suggestions:
                        results!.items!.map((item) => item.title).toList(),
                    onTap: (_) {},
                  ),
              ])))
            ]),
  ...
  Future<void> _suggest() async {
    ...
    final _results = await resultWithSession.result;
    setState(() {
      results = _results;
    });
  ...
  List<Widget> _getList() {
    ...
    return [
      SearchSuggestionsTable(
        suggestions: results
            .expand<String>(
                (result) => result.items?.map((item) => item.title) ?? [])
            .toList(),
        onTap: (_) {},
      ),
    ];
